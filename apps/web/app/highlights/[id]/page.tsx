...
async function fetchBookDetail(documentId: string) {
...
type HighlightsDetailPageProps = {
  params: { id: string } | Promise<{ id: string }>;
};

export default async function HighlightsDetailPage({ params }: HighlightsDetailPageProps) {
  const resolvedParams = await Promise.resolve(params);
  const detail = await fetchBookDetail(resolvedParams.id);
  if (!detail) {
    return (
      <main className="reader-main documents-page">
        <header className="reader-header documents-header">
          <div className="reader-kicker">Highlights</div>
          <h1 className="reader-title">Not found</h1>
        </header>
        <div className="empty-state">Unable to load that book.</div>
      </main>
    );
  }
  ...
  const readHref = `/books/${detail.document.id}?sectionKey=${defaultSectionKey}`;
